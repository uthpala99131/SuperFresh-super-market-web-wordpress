  ...
  class ADMINIFY_Shortcoder extends ADMINIFY_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        ADMINIFY::$shortcode_instances[] = wp_parse_args( array( 'name' => 'adminify/'. $name, 'modal_id' => $this->unique ), $this->args );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( ADMINIFY::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary adminify-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. esc_html( $value['button_title'] ) .'</a>';
      }
