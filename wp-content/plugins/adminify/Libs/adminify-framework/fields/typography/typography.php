  ...
  class ADMINIFY_Field_typography extends ADMINIFY_Fields {
        ...
        echo '<div class="adminify--blocks adminify--blocks-selects">';
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo $this->create_select( apply_filters( 'adminify_field_typography_backup_font_family', array(
              ...
              'Georgia, serif',
              'Palatino Linotype'
            ) ), 'backup-font-family', esc_html__( 'Default', 'adminify' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_align'] ) ) {
            ...
            ), 'text-align', esc_html__( 'Default', 'adminify' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_variant'] ) ) {
            ...
            ), 'font-variant', esc_html__( 'Default', 'adminify' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_transform'] ) ) {
            ...
            ), 'text-transform', esc_html__( 'Default', 'adminify' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_decoration'] ) ) {
            ...
            ), 'text-decoration', esc_html__( 'Default', 'adminify' ) );
            echo '</div>';
          }
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'adminify-webfontloader' ) ) {
        ...
        $customwebfonts = apply_filters( 'adminify_field_typography_customwebfonts', array() );
        ...
        $webfonts['safe'] = array(
          ...
          'fonts' => apply_filters( 'adminify_field_typography_safewebfonts', array(
            'Arial',
            'Arial Black',
            'Helvetica',
            'Times New Roman',
        ...
        $webfonts['google'] = array(
          ...
          'fonts' => apply_filters( 'adminify_field_typography_googlewebfonts', adminify_get_google_fonts()
        ) );

        $defaultstyles = apply_filters( 'adminify_field_typography_defaultstyles', array( 'normal', 'italic', '700', '700italic' ) );

        $googlestyles = apply_filters( 'adminify_field_typography_googlestyles', array(
          '100'       => 'Thin 100',
        ...
        wp_localize_script( 'adminify', 'adminify_typography_json', array(
          'webfonts'      => $webfonts,
    ...
    public function enqueue_google_fonts( $method = 'enqueue' ) {
      ...
      if ( $is_google ) {
        ...
        if ( $font_weight || $font_style ) {
          ...
          ADMINIFY::$webfonts[$method][$font_family] = array();
        }
